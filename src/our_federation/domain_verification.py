    ...
    def remove(self, challenge_id: str) -> None:
        ...
        if challenge_id in self._challenges:
            ...
            if challenge.domain in self._by_domain:
                self._by_domain[challenge.domain] = [
                    cid for cid in self._by_domain[challenge.domain] if cid != challenge_id
                ]
...
class AttestationStore:
    ...
    def get_for_domain(
        ...
        attestations: list[DomainAttestation] = [
            a for a in (self._attestations.get(aid) for aid in attestation_ids) if a is not None
        ]
...
async def create_challenge(
    ...
    logger.info(
        f"Created domain verification challenge for {domain} (federation={federation_did}, expires={challenge.expires_at})"
    )
...
async def verify_domain(
    ...
    if not verified and trust_score >= auto_verify_threshold and successful_methods:
        logger.info(
            f"High trust ({trust_score:.2f}) for {federation_did}: auto-verifying with {len(successful_methods)} method(s)"
        )
        verified = True
...
async def _verify_mutual_attestation(
    ...
    if subject_trust < LOW_TRUST_THRESHOLD:
        ...
        logger.debug(
            f"Low trust ({subject_trust:.2f}) for {subject_did}: reduced attestation chain length to {max_chain_length}"
        )
    ...
    for attestation in attestations:
        ...
        if trust >= min_trust:
            logger.debug(
                f"Domain {domain} verified via attestation from {attestation.attester_did} (trust={trust:.2f})"
            )
        ...
        if attestation.chain and len(attestation.chain) < max_chain_length:
            ...
            for chain_did in attestation.chain:
                ...
                if chain_trust >= min_trust:
                    logger.debug(
                        f"Domain {domain} verified via transitive attestation through {chain_did} (trust={chain_trust:.2f})"
                    )
                    return True, attestation
...
def verify_domain_sync(
    ...
    return asyncio.run(
        verify_domain(domain, federation_did, local_did, methods, require_all, use_cache, trust_threshold)
    )
