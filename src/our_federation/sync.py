...
def _create_auth_headers(
    ...
    if not did or not private_key_hex:
        raise RuntimeError(
            "Federation credentials not configured. Set VALENCE_FEDERATION_NODE_DID and VALENCE_FEDERATION_PRIVATE_KEY."
        )
                    ...
                    if response.status == 200:
                        ...
                        if changes:
                            ...
                            beliefs_received = await self._process_sync_changes(
                                node_id, trust_level, changes, conflict_detected=conflict_detected
                            )
