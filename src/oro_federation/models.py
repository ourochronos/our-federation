...
class BeliefProvenance:
    ...
    def from_row(cls, row: dict[str, Any]) -> BeliefProvenance:
        ...
        return cls(
            ...
            federation_id=(
                row["federation_id"] if isinstance(row["federation_id"], UUID) else UUID(row["federation_id"])
            ),
            origin_node_id=(
                row["origin_node_id"] if isinstance(row["origin_node_id"], UUID) else UUID(row["origin_node_id"])
            ),
            origin_belief_id=(
                row["origin_belief_id"] if isinstance(row["origin_belief_id"], UUID) else UUID(row["origin_belief_id"])
            ),
