...
class KeyRotationManager:
    ...
    def _prune_previous_keys(self) -> None:
        ...
        # Remove expired keys
        self._previous_keys = [
            k for k in self._previous_keys if not k.revoked and (k.expires_at is None or k.expires_at > now)
        ]
    ...
    def _audit(
        ...
        entry = AuditEntry(
            ...
            source_gateway_endpoint=(
                self._external_gateways.get(source_federation_id) if source_federation_id else None
            ),
            target_federation_id=target_federation_id,
            target_gateway_endpoint=(
                self._external_gateways.get(target_federation_id) if target_federation_id else None
            ),
            direction=direction,
            belief_ids=belief_ids or [],
            belief_count=len(belief_ids) if belief_ids else 0,
        ...
        return {
            ...
            "stats": {
                "last_hour": {
                    ...
                    "outbound_shares": len(
                        [e for e in recent_entries if e.event_type == AuditEventType.OUTBOUND_SHARE]
                    ),
                ...
                "last_day": {
                    ...
                    "success_rate": (
                        len([e for e in daily_entries if e.success]) / len(daily_entries) if daily_entries else 1.0
                    ),
                },
            },
            "rate_limits": {
